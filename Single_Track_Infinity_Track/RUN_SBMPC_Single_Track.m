%% Sampling-Based MPC
% Initialization of the Sampling-Based MPC problem for a nonlinear
% single-track model driving in a eight/infinity shaped track
% notice that this last implementation the gear are also controlled by the
% SBMPC and not heuristically
clc
clear all;
close all;
warning off
...
param.timesteps = 25; %Horizon(in the paper its called timesteps)
param.samples = 50;  %Number of samples
param.iterations = 5000; % Number of iterations
param.xmeasure = [-32.5 250 0 0 pi/2.5 0 0 0 0 0];    %Initial conditions (x-position,x-velocity,angle,anuglar velocity)
param.x_desired = [0 0 30 0 0 0 0 0 0 0];    %Desired states (x-position,x-velocity,angle,anuglar velocity)
param.tmeasure = 0;    % Initial time
param.dT = .1; % Timestep
n_inputs = 5; %number of inputs

param.u = 0*ones(param.timesteps,n_inputs); % Initial input sequence
% param.u=normrnd(0,20,[param.timesteps,n_inputs]);
param.u(:,4) = 0.2;
param.u_UB = [0.5263 1 1 1 5]; %Control upper bound
param.u_LB = [-0.5263 0 0 0.1 1]; %Control lower bound
param.system_noise = [0.01 0.01 0.01 0.01 0.01 0.01 0.01 0.01 0.01 0.01];


% Control hyperparameters
param.lambda = 2; %Inverse temperature
param.sigma = [.01 0.15 .05 .1 1]; %Covariance
param.alpha = 0.01; %Base distribution parameters (0 < alpha < 1)
param.gamma = param.lambda*(1-param.alpha);

tic
inital_time_cpu = cputime;
[t_all, x_all, u_all] = SBMPC_SingleTrack(@system_dynamics, param, @running_cost,@terminal_cost);
param.time_normal = toc
param.cpu_time = cputime - inital_time_cpu

function [J] = running_cost(x, x_desired,t)
% commented costs for inspiration
% calculate the cost
% Q = diag([0,0,1,10,0,0,0,0,0,0]);
Q = diag([0,0,1,50,0,0,0,0,0,0]);
% Q = diag([0,0,0.5,50,0,0,0,0,0,0]);
...
%    J = J + 100000000000; 
% end
% if sqrt(((x-x_desired)*Q*(x-x_desired)')^2) > 7
%    J = J + 100000000000;
% end
...
% if sqrt((x(1)-x_desired(1))^2+(x(2)-x_desired(2))^2) < 0.2
%     J = J + 1*500000*(7.5-x(3))^2;
% else
%     J = J + 1*50000*(10-x(3))^2; 
% end
%     J = J/(10^(t*0.1));
% J= J + 20000*(2-sqrt(x(1)^2+x(2)^2))^ 2;
% J = J + 100*(sqrt(x_desired(2)^2+x_desired(4)^2)-2)^2;
%  J = 100*(x(1))^2 ...
%  + 500*(1 + cos(x(3)+pi))^2 ...
%  + 1*(x(2))^2 + 15*(x(4))^2;
end
...
J =0*(x-x_desired)*Q*(x-x_desired)';

end

function single_track = system_dynamics(t, X ,U, dW)
%% Gear input empirically tested
% G = 1;
% if X(3) < 9
%   G = 1;
...
% Back transformation of the control input
F_b = 15000*F_b;
...
single_track=[x_dot;y_dot;v_dot;beta_dot;psi_dot;omega_dot;x_dot_dot ...
            ;y_dot_dot;psi_dot_dot;varphi_dot_dot] + dW'; % left-hand side
